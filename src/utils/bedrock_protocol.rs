...
pub fn read_response(buffer: &mut Bytes) -> Result<BedrockPing, PingError> {
    let _packet_id = buffer.try_get_u8()
        .map_err(|_| PingError::ReadPacketError("Packet id not found".to_string()))?;

    let _timestamp = buffer.try_get_u64()
        .map_err(|_| PingError::ReadPacketError("Timestamp not found".to_string()))?;

    let _server_guid = buffer.try_get_u64()
        .map_err(|_| PingError::ReadPacketError("Server guid not found".to_string()))?;

    let _magic = buffer.try_get_u128()
        .map_err(|_| PingError::ReadPacketError("Magic id not found".to_string()))?;


    let string_size = buffer.try_get_u16()
        .map_err(|_| PingError::ReadPacketError("String size not found".to_string()))?;

    let string_data = try_split_to(buffer, string_size as usize)
        .ok_or(PingError::ReadPacketError("String data not found".to_string()))?;
